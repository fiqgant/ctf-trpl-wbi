...
async function main(): Promise<void> {
  ...
  const wasmPath = path.join(distDir, 'level09.wasm');

  await mkdir(distDir, { recursive: true });
  compileAssemblyScript(source, wasmPath);

  const salt = requireFlagSalt(process.env);
  const flagCommitment = await deriveFlagCommitment('L09', 'AXIOM-DELTA-9371-OMEGA-CODE', salt);
  const sha256 = await sha256File(wasmPath);

  await writeFile(
    path.join(distDir, 'notes.txt'),
    ['Firmware target: wasm32', 'Exported function: validate(input: string) -> i32', `sha256=${sha256}`].join('\n'),
    'utf8'
  );

  await writeFile(
    path.join(distDir, 'manifest.json'),
    JSON.stringify({ level: 'L09', sha256, flagCommitment }, null, 2),
    'utf8'
  );
}
